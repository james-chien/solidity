...
{
	checkYulIdentifierName(_function.name, _function.location);
	for (yul::TypedName varName: _function.parameters + _function.returnVariables)
		checkYulIdentifierName(varName.name, varName.location);
...
void ReferencesResolver::operator()(yul::VariableDeclaration const& _varDecl)
{
	...
	{
		checkYulIdentifierName(identifier.name, identifier.location);
...
void ReferencesResolver::checkYulIdentifierName(yul::YulString _name, SourceLocation const& _location)
{

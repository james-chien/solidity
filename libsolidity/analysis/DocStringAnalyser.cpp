...
CallableDeclaration const* findBaseCallable(std::set<CallableDeclaration const*>& _baseFuncs, ContractDefinition const& _contract)
{
	for (CallableDeclaration const* baseFuncCandidate: _baseFuncs)
		if (baseFuncCandidate->annotation().contract->id() == _contract.id())
			return baseFuncCandidate;
		else if (auto callable = findBaseCallable(baseFuncCandidate->annotation().baseFunctions, _contract))
			return callable;
...
{
	if (_annotation.inheritContract == nullptr)
		return nullptr;

	if (auto const callable = findBaseCallable(_baseFuncs, *_annotation.inheritContract))
		return callable;
	...
	m_errorReporter.docstringParsingError(
		...
		_annotation.inheritContract->name() +
